 ...
 */
public class Excel_Reporte_Programas extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if (ask.equals("programas")) {
            try {
                ...
                System.err.println("ERROR BD: Programas reporte, consultando cantidad de universidades y/o periodo activo " + ex.getMessage());
                con.Desconectar();
            } finally {
                con.Desconectar();
            }
            ...
            try {
                limpiarBasurero();
                System.out.println("Generando Programas...");
                /*LISTAS NECESARIAS*/
                ListaUniversidades univs = new ListaUniversidades(periodo);
                ListaNiveles niveles = new ListaNiveles(false);
               
                ExcelRepProgramas programas = new ExcelRepProgramas(anio, obj, periodo, univs, niveles);
                obj = programas.hacerRepProgramas();
                programas = null;
        
                univs = null;
                niveles = null;
                /*IMPRIMIR EL EXCEL*/
                EnviarExcel(response, obj.getLibro(), "Programas " + anio + "- " + (Integer.parseInt(anio) + 1));
                obj = null;
                limpiarBasurero();
    ...
    private void EnviarExcel(HttpServletResponse response, Workbook libro, String nombreArchivo) {
        try {
            System.out.println("Generando exportacion Programas...");
            ...
            fileOut.flush();
            fileOut.close();
            System.out.println("Programas Generada");
        } catch (Exception e) {
