 ...
 */
public class VinculacionIn31Action extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        VinculacionIn31Form frm = (VinculacionIn31Form) form;
        if (!frm.getValores().equals("-1")) {
            if (frm.getValores().equals("NOAPLICA")) {
                ConexionMySQL conexion = new ConexionMySQL();
                String query = "UPDATE indicadores_aplica SET aplica='0' WHERE id_universidad="
                        .concat(idUniversidad)
                        .concat(" and indicador='31' AND id_periodo=").concat(idPeriodo);
                if (conexion.Modificar(query) == 0) {
                    query = "INSERT INTO indicadores_aplica (id_universidad,id_periodo,id_nivel,aplica,indicador) VALUES("
                            .concat(idUniversidad).concat(", ").concat(idPeriodo).concat(",-1,'0','31')");
                    conexion.Insertar(query);
                } else {
                    //eliminar los datos de la tabla vinculacionin31
                    query = "DELETE FROM vinculacionin31 WHERE id_universidad="
                            .concat(idUniversidad)
    ...
    private boolean Guardar(String valores, String idUniversidad, String idPeriodo) {
        ...
        try {
            ...
            if (conexion.Modificar(query) == 0) {
                query = "INSERT INTO indicadores_aplica (id_universidad,id_periodo,id_nivel,aplica,indicador) VALUES("
                        .concat(idUniversidad).concat(", ").concat(idPeriodo).concat(",-1,'1','31')");
                conexion.Insertar(query);
            }
