        ...
        <%
            ...
            try {
                ...
                if (rs.next()) {
                        ...
                        cols++;
                         matricula_t = "select matricula from mat_total_universidad where id_universidad=".concat(idUniversidad).concat(" and id_periodo =".concat(idPeriodo).concat(" and id_nivel=").concat(rs.getString("id_nivel")));
                          ...
                          while (resmat.next()) {
                            ...
                            mat[z]=resmat.getDouble(1);
                            summat+=resmat.getDouble(1);
                         }
                    ...
                    z=0;
                    while(rs.next()){
                        tablas = tablas.concat("<td align='center'><input readonly=\"readonly\" id=\"etiqueta\" type='text' value='"+Math.rint((mat[z]/summat*100)*100)/100+"'></td>");
                        z++;
                    }
                    tablas = tablas.concat("<td align='center'><input readonly=\"readonly\" id=\"etiqueta\" type='text' value='100'></td>");
                    tablas = tablas.concat("</tr>");
                    tablas = tablas.concat("<tfoot>");
                    tablas = tablas.concat("<tr>");
                    tablas = tablas.concat("<td colspan='".concat(String.valueOf(cols+1)).concat("' class='rounded-foot-left'>Notas: &Eacute;ste cuadro es llenado autom&aacute;ticamente al completar los cuadros siguientes.</td>"));
                ...
                while (rs.next()) {
                    ...
                    String sql2 = "select id_pe, nombre_programa, version from programa_educativo where id_nivel=".concat(id_nivel).concat(" and id_pe in(select id_pe from pe_universidad where id_universidad=".concat(idUniversidad).concat(" and id_periodo=".concat(idPeriodo).concat(")")));
                    ...
                    while (rs2.next()) {
                        ...
                        if (rs3.next()) {
                            tablas = tablas.concat("<tr>");
                            tablas = tablas.concat("<td align='center'>".concat(rs2.getString("nombre_programa")+" ("+rs2.getString("version")+")").concat("</td>"));
...
<%      //SE CONSULTA INDICADOR 
        String consultacom = "SELECT comentario FROM comentarios WHERE "
            ...
            + "id_periodo="+ sesion.getAttribute("idPeriodo").toString()+" AND indicador=34";
