...
<html:form action="/EficaciaIn7">
    ...
    int muestreo = 0;
            ...
            try {
                //NÚMERO DE NIVELES
                String sql = "Select distinct(pe.id_nivel),descripcion from programa_educativo pe inner join nivel_pe n on pe.id_nivel = n.id_nivel where pe.id_pe in (select id_pe from pe_universidad where id_universidad =".concat(sesion.getAttribute("idUniversidad").toString()).concat(" and id_periodo =".concat(String.valueOf(idPeriodo)).concat(") order by pe.id_nivel"));
                ...
                ResultSet resp;
// 
                String sql3 = "select year(fecha_inicio) as fecha_inicio from periodos where id_periodo='".concat(sesion.getAttribute("idPeriodo").toString()).concat("'");
                ResultSet rs3 = con.Consultar(sql3);
                if (rs3.next()) {
                    anio = rs3.getInt("fecha_inicio");
                }
                String anios = "";
                //AÑOS ESCOLARES
                // Calendar ahoraCal = Calendar.getInstance();
                while (rs.next() && (rs.getString("id_nivel").equals("1") ||rs.getString("id_nivel").equals("2") ||rs.getString("id_nivel").equals("3") )) {//while
                    //se realiza una consulta para validar si existen datos en el indicador 5
                    sql4 = "select sum(egresados) as suma from eficaciain5 where id_universidad='".concat(String.valueOf(idUniversidad)).concat("' and id_periodo ='").concat(String.valueOf(idPeriodo)).concat("'and id_nivel = '").concat(String.valueOf(rs.getInt("id_nivel"))).concat("'");
                    rs4 = con.Consultar(sql4);
                    rs4.next();
                    if (rs4.getString("suma") != null) {
                        volver = 1;
                        String cons = "select id_pregunta, pregunta from encuesta_preguntas where id_encuesta=(select id_encuesta from encuestas where id_nivel=".concat(rs.getString("id_nivel")).concat(" and encuesta_preguntas.id_encuesta=encuestas.id_encuesta and indicador=7) and activo=1");
                       
                        ResultSet preg = con.Consultar(cons);
                        if (rs.getString("id_nivel").equals("3")) {
                            tablas = tablas.concat("<h2 align='left'>".concat("Cuadro 7.1.".concat(rs.getString("id_nivel"))).concat("<br/>Egresados del Nivel ").concat(rs.getString("descripcion")).concat(" Satisfechos ").concat(String.valueOf(anio+1)).concat("</h2>"));
                            anios = "2";
                        } else {
                            
                            tablas = tablas.concat("<h2 align='left'>".concat("Cuadro 7.1.".concat(rs.getString("id_nivel"))).concat("<br/>Egresados del Nivel ").concat(rs.getString("descripcion")).concat(" Satisfechos ").concat(String.valueOf(anio)).concat("</h2>"));
                            anios = "1";
                        }
                        //TABLA
                        ...
                        tablas = tablas.concat("<th rowspan='3' class='rounded-q1'>DESCRIPCIÓN</th>");
                        ...
                        tablas = tablas.concat("</thead>");
                        //CUERPO
                        tablas = tablas.concat("<input type='hidden' name='id_niv" + c + "' value='".concat(rs.getString("id_nivel")).concat("' />"));
                        while (preg.next()) {                           //PREGUNTAS                           
                            ...
                            if (resp.next()) {
                                ...
                                tablas = tablas.concat("<tr>");
                                tablas = tablas.concat("<td width='5%' align='center'>".concat(String.valueOf(no)).concat("</td>")); //NÚMERO CONSECUTIVO
                    ...
                    tablas = tablas.concat("<tr>");
                         tablas = tablas.concat("<td align='center' style='font-weight: bold' colspan='2' class='rounded-foot-left'>DISTRIBUCIÓN PORCENTUAL</td>");
                    ...
                    tablas = tablas.concat("</tr>");
                        sql4 = "select sum(egresados) as suma from eficaciain5 where id_universidad='".concat(String.valueOf(idUniversidad)).concat("' and id_periodo ='").concat(String.valueOf(idPeriodo)).concat("'and id_nivel = '").concat(String.valueOf(rs.getInt("id_nivel"))).concat("' and anio=").concat(anios);
                        rs4 = con.Consultar(sql4);
                        if (rs4.next()) {
                            poblacion = rs4.getInt("suma");                            
                        }
                        if (poblacion >= 2835) {
                        ...
                        tablas = tablas.concat("<tr>");
                        tablas = tablas.concat("<td colspan='13' class='rounded-foot-left'>Notas: Se encuestará al menos a ").concat(String.valueOf(muestreo)).concat(" personas, ya que lo poblacion total es de ").concat(String.valueOf(poblacion)).concat(". Atención, por lo anterior la columna 'H' debe ser igual o mayor a la cantidad mencionada.</td>");
                ...
                System.err.print("error eficacia 7 jsp: " + ex);
            } finally {
                con.Desconectar();
            }
        %>
...
<%      //SE CONSULTA INDICADOR 
        String consulta = "SELECT comentario FROM comentarios WHERE "
            ...
            + "id_periodo="+ sesion.getAttribute("idPeriodo").toString()+" AND indicador=7";
