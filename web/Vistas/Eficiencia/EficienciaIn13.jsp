    ...
    <div id="efici13" align="center">
        <%
            ...
            try {
                ...
                if (rs3.first()) {
                    matutino = rs3.getString("turno_matutino");
                    vespertino = rs3.getString("turno_vespertino");
                }
                ...
                if (rs4.next()) {
                    total_alumnos = rs4.getString("matricula_total");
                }
                ...
                tablas = tablas.concat("<th scope='col' class='rounded-company'>Descripción de los Edificios, Laboratorios y Talleres</th>");
                ...
                tablas = tablas.concat("<th class='rounded-q1'>Capacidad del espacio académico <br/>ocupado por otras áreas de trabajo</th>");
                ...
                while (rs1.next()) {
                    ...
                    String sql2 = "SELECT no_laboratorios,espacio_academico,observaciones FROM eficienciain13 WHERE id_edifi='".concat(rs1.getString("id_edificio")).concat("' and id_universidad=".concat(sesion.getAttribute("idUniversidad").toString()).concat(" and id_periodo=").concat(sesion.getAttribute("idPeriodo").toString()));
                    ResultSet rs2 = con.Consultar(sql2);
                    contenedor = true;
                    if (rs2.next()) {
                        tablas = tablas.concat("<tr>");
                        tablas = tablas.concat("<td align='center'>").concat(rs1.getString("descripcion")).concat("</td>");
                        tablas = tablas.concat("<td  align='center' ><input readonly=\'readonly\' id='etiqueta' style='width:90% type='text'  name='val_uni").concat(rs1.getString("id_edificio")).concat("' value='").concat(rs1.getString("capacidad")).concat("' onblur='EficienciaIn13(this);' ></td>");
                        tablas = tablas.concat("<td align='center' ><input type='text' name='val_uni' maxlength='6' value='").concat(rs2.getString("no_laboratorios")).concat("' onblur='EficienciaIn13(this);' ></td>");
                        ...
                        tablas = tablas.concat("<td align='center' ><input type='text' maxlength='6' name='val_cap' value='").concat(rs2.getString("espacio_academico")).concat("' onblur='EficienciaIn13(this);' ></td>");
                        tablas = tablas.concat("<td align='center' ><input style='width:90%' maxlength='57' type='text' name='val_obser' value='").concat(rs2.getString("observaciones")).concat("' ></td>");
                        tablas = tablas.concat("</tr>");
                    } else {
                        tablas = tablas.concat("<tr>");
                        tablas = tablas.concat("<td align='center'>").concat(rs1.getString("descripcion")).concat("</td>");
...
<%      //SE CONSULTA INDICADOR 
        String consultacom = "SELECT comentario FROM comentarios WHERE "
            ...
            + "id_periodo="+ sesion.getAttribute("idPeriodo").toString()+" AND indicador=13";
