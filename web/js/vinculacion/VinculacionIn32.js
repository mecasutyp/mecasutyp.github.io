function guardar32(){
    ...
    if ($("form[name='VinculacionIn32Form'] input").hasClass("inputerror")){ //SI TIENE ERRORES
        ...
        //SE GUARDARA LOS COMENTARIOS
           var comentario = $("form[name='VinculacionIn32Form'] input[name='comentario']").val();
...
function operaciones_32(){
    ...
    for(var i=1;i<c;i++){
        ...
        if(parseInt($("form[name='VinculacionIn32Form'] input[name='valEgresados_"+i+"']").val())>0 && parseInt($("form[name='VinculacionIn32Form'] input[name='valPlazas_"+i+"']").val())>0 ){        
            $("form[name='VinculacionIn32Form'] input[name='BT"+i+"']").val(getDecimal((parseFloat($("form[name='VinculacionIn32Form'] input[name='valEgresados_"+i+"']").val())/parseFloat($("form[name='VinculacionIn32Form'] input[name='valPlazas_"+i+"']").val()))*100) + " %");
        } else{
