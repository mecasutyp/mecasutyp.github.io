        ...
        buttons: {
            ...
            "Exportar Reporte en Excel": function(){
                ...
                if($("select[name='report']").val()=="cat_Eficacia"){
                    window.open('Excel_Reporte_Categorias.msut?ask='+$('#report').val()+'&ipe='+$('#perio').val());
                }
                ...
                if($("select[name='report']").val()=="cat_Vinculacion"){
                    window.open('Excel_Reporte_Categorias.msut?ask='+$('#report').val()+'&ipe='+$('#perio').val());
                }
                if($("select[name='report']").val()=="cat_Equidad"){
                    window.open('Excel_Reporte_Categorias.msut?ask='+$('#report').val()+'&ipe='+$('#perio').val());
                }
    ...
    $.ajax({
        ...
        success: function(response){
           ...
           if(response==1){
                ...
                $().alerta('rojo','Error al Guardar los Datos aqui es el error');               
            }  
        },
        failiure: function(response){
...
function base64(file, callback){///METODO QUE CODIFICA UN ARCHIVO A BASE 64 PARA ENVIAR
    var coolFile = {};
...
function enviarcorreo(archivo, universidad, indicador, extension, iduniversidad, idperiodo){
        
          $.ajax({ //Comunicación jQuery hacia JSP
            type: "POST",
            url: "Vistas/Admon/enviarcor.jsp",
            data: "archivo="+encodeURIComponent(archivo)+"&extension="+extension+"&universidad="+universidad
                +"&indicador="+indicador+"&IdUni="+iduniversidad+"&IdPer="+idperiodo,
                
            success: function(msg){
                $().alerta('verde', 'El archivo fue enviado.');
            },
            error: function(xml,msg){
               $().alerta('rojo','Error en el envio del correo, intentalo nuevamente.'); 
           }
        });         
}
function verificararchivo(nombreform){
    var filexext=$("form[name='"+nombreform+"'] input[name='archivo']")[0].value;
    if(filexext!=""){
            var extensiones_permitidas = new Array(".pdf");
            var archivo = $("form[name='"+nombreform+"'] input[name='archivo']")[0].value;
       ...
       //compruebo si la extensión está entre las permitidas 
             ...
             if(!extensioncorrecta ){
                  $("form[name='"+nombreform+"'] input[name='archivo']").removeClass("inputok").addClass("inputerror");
             }else{
                  $("form[name='"+nombreform+"'] input[name='archivo']").removeClass("inputerror").addClass("inputok");
             }   
        return true;
    }else{
        return false;
...
function GuardarComentarios(comentari, indicad){
     $.ajax({
            data: { "Comentario": comentari , "Indicador": indicad},
            url: "ComentarioAction",
            type: "post",
            beforeSend: function(xhr){
            },
            success: function(response){
...
function getDecimal(decimal){ //METODO QUE REDONDEA A UNA DECIMA
