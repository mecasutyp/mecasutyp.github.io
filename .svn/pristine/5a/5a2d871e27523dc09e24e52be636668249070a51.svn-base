<%-- 
    ...
    Created on : 24/05/2012, 02:44:12 PM
    Author     : Joshua
--%>
...
<tiles:insert template="/Vistas/Master.jsp">
    <%
        ...
        String sql = "Select archivo_jsp, archivo_js from system_indicadores where id_categoria = 4 and activo = 1";
        ResultSet rs = conexion.Consultar(sql);
        int i = 1;
    %>
    <tiles:put name="title" type="string">
        MECASUT - Vinculaci&oacute;n
    </tiles:put>
    <tiles:put name="scripts" type="string">
        ...
        %>
        <script language="javascript" type="text/javascript" src="js/vinculacion/<%=js%>"></script>
        <%
            }
        %>
    </tiles:put>
    <tiles:put name="body" type="string">
        <h1 style="text-align: center;">IV. VINCULACI&Oacute;N</h1>
        <div id="tabs">
            <ul>
                <%
                    i = 1;
                    rs.beforeFirst();
                    while (rs.next()) {
                        String tab = "<li><a id='pest".concat(String.valueOf(i)).concat("' onclick='limpiarDivs(); sendRequest(\"GET\",\"Vistas/Vinculacion/") + rs.getString("archivo_jsp") + ".jsp\",\"indT".concat(String.valueOf(i)).concat("\",\"" + rs.getString("archivo_jsp") + "Form\",\"Indicador 4.").concat(String.valueOf(i)).concat("\");' href='#indT").concat(String.valueOf(i)).concat("'>Indicador 4.").concat(String.valueOf(i)).concat("</a></li>");
                        out.print(tab);    
                        i++;
                    }
                %>
        ...
        <input type="hidden" id="indActual" value="vinculacion"/>
