...
function retrieveURL(url,nameOfFormToPost,sesion) {
    ...
    if (window.XMLHttpRequest) { // Non-IE browsers
        ...
        try {
            ...
            alert("Problema en la respuesta del servidor:\n Es probable que los datos no se hayan guardado correctamente debido a un error en el envio, por favor verifique que tenga conexion a internet y vuelva a intentarlo.\n"+e);
...
function processStateChange() {
    ...
    if (req.readyState == 4) { // Complete
        if (req.status == 200) { // OK response
            ...
            alert("Problema en la respuesta del servidor:\n Es probable que los datos no se hayan guardado correctamente debido a un error en el envio, por favor vuelva a intentarlo. \n" + req.statusText);
            $( "#dialog-procesando" ).dialog("close");
        }
    }
}
...
function splitTextIntoSpan(textToSplit){
    ...
    //Split the document
    returnElements=textToSplit.toString().split("</span>")
...
function replaceExistingWithNewHtml(newTextElements){
    ...
    for ( var i=newTextElements.length-1; i>=0; --i ){
        ...
        if(newTextElements[i].indexOf("<span")>-1){
            ...
            //alert("startnamepos:"+startNamePos+"\nendnamePos: "+endNamePos+"\nname: "+name+"\nstartContentPos: "+startContentPos+"\nContent: "+content);
