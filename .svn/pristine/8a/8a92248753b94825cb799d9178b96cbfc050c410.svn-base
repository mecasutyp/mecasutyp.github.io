    ...
    private boolean Guardar(String valores, String idUniversidad, String idPeriodo) {
        ...
        try {
            for (int i = 1; i < values.length; i++) {
                ...
                String sql = "Update vinculacionin29 set capacitacion = ".concat(datos[1]).concat(", actualizacion = ").concat(datos[2]).concat(", desarrollo_p = ").concat(datos[3]).concat(", total_egresados = ").concat(datos[4]).concat(" where id_universidad = ").concat(idUniversidad).concat(" and id_periodo = ").concat(idPeriodo).concat(" and id_nivel = ").concat(datos[0]);
